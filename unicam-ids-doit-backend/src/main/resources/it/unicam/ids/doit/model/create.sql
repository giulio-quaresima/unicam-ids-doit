...
-- at 2021-06-06T18:50:17.845198Z                --
---------------------------------------------------
    ...
    create table candidatura_invito (
       candidatura_id integer not null,
        inviti_id integer not null,
        primary key (candidatura_id, inviti_id)
    );
    ...
    create table soggetto_candidatura (
       soggetto_id integer not null,
        candidature_id integer not null,
        primary key (soggetto_id, candidature_id)
    );

    create table soggetto_invito (
       soggetto_id integer not null,
        inviti_ricevuti_id integer not null,
        primary key (soggetto_id, inviti_ricevuti_id)
    );
    ...
    alter table candidatura_invito 
       add constraint UK_ecmvlfhopgmov21gqcoltyu0v unique (inviti_id);

    alter table soggetto_candidatura 
       add constraint UK_t6s8jnnbcn4dgiaho52c5ajx8 unique (candidature_id);

    alter table soggetto_invito 
       add constraint UK_26rk84yua9bpq2gifp26b8dwp unique (inviti_ricevuti_id);
    ...
    alter table candidatura_invito 
       add constraint FKl85x2ybqm2yb7gqroatounnf8 
       foreign key (inviti_id) 
       references invito;

    alter table candidatura_invito 
       add constraint FKkj43jwbu3t7s8q9p599whgrrv 
    ...
    alter table soggetto_candidatura 
       add constraint FKt0n2q9vnybnjbr2w655aiqlm0 
       foreign key (candidature_id) 
       references candidatura;

    alter table soggetto_candidatura 
       add constraint FKhjr9hs9o93oxj5m2nv9bhk443 
       foreign key (soggetto_id) 
       references soggetto;

    alter table soggetto_invito 
       add constraint FKh4ol6f3urmln3hkbnquk7o64t 
       foreign key (inviti_ricevuti_id) 
       references invito;

    alter table soggetto_invito 
       add constraint FKg6w1bikdmh5dat1lyx2hyx2yd 
       foreign key (soggetto_id) 
       references soggetto;
